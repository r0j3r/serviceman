...
struct svc_packet * parse_def(char * buff);
struct svc_packet * parse_def_file(char * filename);
int next_char(void);
...
parse_def(char * buff)
{
    ...
    staging_buff_init();
    packet = (struct svc_packet *)staging_buffer_alloc(sizeof(*packet));
    ...
    while(token_lookahead != END_OF_FILE)
    {
        ...
        else if (token_lookahead == PROGRAM)
        {
            ...
            if (token_lookahead == STRING)
            {
                packet->exec_file_path_offset = lex_start_pos;
                token_lookahead = next_tok();
            } 
        }
        else if (token_lookahead == PROGRAMARGS)
        {
            ...
            p_args_staging = staging_buffer_alloc(sizeof(int) * (p_argc_count + 1));
            if (!p_args_staging)
            {
                return 0;
            }
            memcpy(p_args_staging, p_args, sizeof(int) * (p_argc_count + 1));
            packet->argv_offset = get_offset(p_args_staging);
            packet->argv_count = p_argc_count;   
        }
        else if (token_lookahead == KEEPALIVE)
        {
            token_lookahead = next_tok();
            packet->keepalive_opts_offset = parse_keepalive_options();
        }
    ...
    packet->stringtab = get_offset(string_tab_staging); 
...
{
    unsigned char keepalive_options[256];
